...
public class VoiceServer
{
		ServerSocket ss;
			...
			//New thread maintaning the list of user name// 
                        		UserList m=new UserList(s2,al2,alname); 
                        		Thread t2=new Thread(m);
                        			t2.start();

			//New thread for receiving and sending the messages 
			SendRecv r=new SendRecv(s,al,alname); 
                        		Thread t=new Thread(r);
                        			t.start();

			//New thread for update the list of username
                        		UpdateUser my=new UpdateUser(s1,al1,s,s2);  
...
class UpdateUser implements Runnable
{
        	...
        	DataInputStream ddin;
       		String sname;
        	UpdateUser(Socket s1,Vector al1,Socket s,Socket s2)
        	{
                		this.s1=s1;
                		this.al1=al1;
                		this.s=s;
        	...
        	public void run()
        	{
				...
				//Remove the logout user name from arraylist
                                			UserList.alname.remove(sname);
                                			UserList.every();
                                			al1.remove(s1);
                                			SendRecv.al.remove(s);
                                			UserList.al2.remove(s2);
                        		}
                		}
                		catch(Exception e){
                		}
...
class UserList implements Runnable
{
	...
	UserList(Socket s2,Vector al2,Vector alname)
        	{
               		this.s2=s2;
                		this.al2=al2;
                		this.alname=alname;
	...
	//send the list of users name to all client
...
//class is used to recieve the message and send it to all clients
class SendRecv implements Runnable
{
	...
	SendRecv(Socket s,Vector al,Vector alname)
        	{
                		this.s=s;
                		this.al=al;
                		this.alname=alname;
